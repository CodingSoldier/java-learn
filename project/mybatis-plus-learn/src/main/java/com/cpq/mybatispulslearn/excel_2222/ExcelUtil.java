...
public class ExcelUtil<T> {
        ...
        if (column != null){
            ...
            for (ExcelProperties.ColTitle colTitle : column){
                if (StringUtils.isNotBlank(colTitle.getText())){
                    ...
                    columnWidth = colTitle.getWidth() != null ? colTitle.getWidth() : columnWidth;
    ...
    public static <T> void exportExcelFile(ExcelProperties properties, List<T> dataList, ExcelCallback<T> callback){

        HSSFWorkbook wb = ExcelUtil.createExcel(properties, dataList, callback);

        String excelName =  StringUtils.isNotBlank(properties.getExcelName()) ? properties.getExcelName() : "excel导出";
        String path = "E:\\" + excelName + ".xls";
        try{
