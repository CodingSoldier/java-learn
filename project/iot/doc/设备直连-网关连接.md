...
本文既定义主题和消息协议，也说明如何在当前 Spring Boot、HiveMQ Client、EMQX 项目中逐步实现。当前代码仍使用 `/sys/servie/invoke` 和 `/sys/servie/invoke_reply`，本文是下一版协议设计，不代表这些主题已经完成迁移。
...
```text
iot/v1/products/{productKey}/devices/{deviceCode}/down/#
```
...
```text
iot/v1/gateways/{gatewayId}/down/#
```
...
sequenceDiagram
    ...
    Gateway->>EMQX: SUBSCRIBE iot/v1/gateways/gw001/down/#
    Web->>Platform: POST /service/invoke
    Platform->>Platform: 生成 msgId 并注册 PendingRequest
...
```text
PUBLISH   iot/v1/products/{自己的productKey}/devices/{自己的deviceCode}/up/#
SUBSCRIBE iot/v1/products/{自己的productKey}/devices/{自己的deviceCode}/down/#
```
...
```text
PUBLISH   iot/v1/gateways/{自己的gatewayId}/up/#
SUBSCRIBE iot/v1/gateways/{自己的gatewayId}/down/#
```
...
HiveMqttReplySubscriber
  -> MqttReplyPayloadHandler
      -> ServiceInvokeReplyService
          -> PendingRequestRegistry
```
...
```text
iot/v1/products/+/devices/+/up/services/+/response
iot/v1/products/+/devices/+/up/properties/set/response
iot/v1/products/+/devices/+/up/properties/report
iot/v1/products/+/devices/+/up/events/+/report
iot/v1/gateways/+/up/sub-devices/services/+/response
iot/v1/gateways/+/up/sub-devices/properties/set/response
iot/v1/gateways/+/up/sub-devices/properties/report
iot/v1/gateways/+/up/sub-devices/events/+/report
iot/v1/gateways/+/up/sub-devices/status/report
iot/v1/gateways/+/up/sub-devices/topology/sync/response
```
...
分发器根据实际 Topic 匹配消息类型，不包含业务处理。网关消息还必须校验 Topic 中的 `gatewayId` 与平台拓扑关系。`MqttUpstreamDispatcher.dispatch(topic, payloadBytes)` 返回 `MqttDispatchResult`，包含字符串 `msgId` 和 `matched`；真实 MQTT callback 忽略返回值，mock 接口直接使用返回值。无法解析 `msgId` 时返回空字符串和 `matched=false`，不得返回 `null`。

### 10.6 Mock 回复接口

保留 `POST /mock/mqtt/invoke-reply`，但请求体改为完整 MQTT Topic 和 payload：

```json
{
  "topic": "iot/v1/gateways/gw001/up/sub-devices/services/switch/response",
  "payload": {
    "msgId": "124549",
    "timestamp": 1781883000100,
    "target": {
...
`MockMqttMessageRequest.topic` 使用 `@NotBlank`，`payload` 使用非空 `Map<String, Object>`。`MockMqttController` 使用 `ObjectMapperUtil` 将 payload 序列化为 UTF-8 JSON 字节，然后调用与真实 HiveMQ 回调相同的 `MqttUpstreamDispatcher.dispatch(topic, payloadBytes)`，并根据返回的 `MqttDispatchResult` 构造响应。禁止直接调用 `PendingRequestRegistry` 或 `ServiceInvokeReplyService`。

接口继续返回 `Result<MockMqttReplyResponse>`，其中包含字符串 `msgId` 和 `matched`。合法但未知或重复的 `msgId` 返回 `matched=false`；Topic、payload 或身份校验失败同样返回 `matched=false` 并记录中文上下文日志。
...
| `MockMqttController` | 改为通过 `MqttUpstreamDispatcher` 进入真实处理链 |
...
- mock reply 使用 `topic + payload` 请求，并经过与真实 MQTT 消息相同的 Topic 解析、身份校验和服务响应处理链。
...
- 单元和 MVC 测试使用 mock MQTT Client 或 Gateway，不依赖真实 EMQX。
