...
/**
 ...
 在Semaphore中，获取是acquire方法，作用是获取一个许可证。         Semaphore.Sync#nonfairTryAcquireShared，计数器减少
 在CountDownLatch中，获取就是await方法，作用是等待直到倒数结束。  CountDownLatch.Sync#tryAcquireShared
 释放方法：释放操作不会阻塞
 在Semaphore中，释放就是release方法，作用是释放一个许可证。计数器增加Semaphore.Sync#tryReleaseShared。
 在CountDownLatch中，获取就是countDown方法，作用是倒数1个数。CountDownLatch.Sync#tryReleaseShared

 AQS的用法
 1、写一个类，想好协作逻辑，实现获取/释放方法
 2、内部写一个Sync类继承AbstractQueuedSynchronizer
 3、独占锁重写tryAcquire/tryRelease
 共享锁重写tryAcquireShared(int acquires)/tryReleaseShared(int release)
 ...
 4、count个线程调用countDown()之后，锁计数器才为0，而前面提到的等待获取共享锁的线程才能继续执行。AbstractQueuedSynchronizer#unparkSuccessor唤醒线程
 ...
 线程未获取到锁，tryAcquire(arg)分成公平与非公平实现，非公平锁实现nonfairTryAcquire(int acquires)，公平锁实现tryAcquire(int acquires)，公平锁中多了一个判断!hasQueuedPredecessors()
 主要是设置state值，并返回是否获取锁成功，如果获取不成功，则将线程入队并等待
